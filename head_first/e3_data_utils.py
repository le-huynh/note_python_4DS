...
from e3_queries import * # `*`: everything
...
def get_swimmers_events(name, age, date):
    ...
    with DBcm.UseDatabase(db_details) as db:
        db.execute(SQL_SWIMMERS_EVENTS_BY_SESSION, 
                   (name, age, date,))
        results = db.fetchall()
    return results
...
def get_swimmers_times(name, age, distance, stroke, date):
    ...
    return a tuple-list of times the swimmer swam on that date
    over the identified distance/stroke combination."""
    with DBcm.UseDatabase(db_details) as db:
